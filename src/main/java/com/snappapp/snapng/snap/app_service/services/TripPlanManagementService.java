...
//    }
    ...
    public List<PlannedTripResponse> getTripsByBusiness(Long userId){
        ...
        return plannedTripService.getPlannedTrips(business).stream().map(PlannedTripResponse::new).toList();
    }

    public List<PlannedTripResponse> getAvailableTrips(Long userId){
        userService.getUserById(userId);
        return plannedTripService.getPlannedTrips().stream().map(PlannedTripResponse::new).toList();
    }
    ...
    public PlannedTripResponse closeTrip(String reference, Long userId){
        ...
        PlannedTrip trip = plannedTripService.update(PlannedTripStatus.CLOSED,reference, business.getCode());
        return new PlannedTripResponse(trip);
    }
    ...
    public List<TripOfferResponse> getAllTripOffers(String tripRef, Long userId) {
        ...
        if(business==null || !business.getCode().equalsIgnoreCase(trip.getBusiness().getCode())){
            throw new ResourceNotFoundException("Trip is not available to this rider");
        }
        return tripOfferService.get(trip).stream().map(TripOfferResponse::new).toList();
    }
    ...
    public List<TripOfferResponse> getAcceptedTripOffers(String tripRef, Long userId) {
        ...
        return tripOfferService.getAccepted(trip).stream().map(TripOfferResponse::new).toList();
    }
    ...
    public TripOfferResponse getOffer(String reference, Long userId) {
        ...
        if(business!=null){
            ...
            if(business.getCode().equalsIgnoreCase(tripOffer.getTrip().getBusiness().getCode())){
    ...
    public TripOfferResponse makeOffer(String reference, Long amount, Long userId){
        ...
        Business business = businessService.getBusinessOfUser(user);
        PlannedTripOffer tripOffer = tripOfferService.get(reference);
        if(business!=null && business.getCode().equalsIgnoreCase(tripOffer.getTrip().getBusiness().getCode())){
            tripOffer = tripOfferService.setRiderOffer(tripOffer.getReference(),amount);
            if(TripOfferStatus.ACCEPTED.equals(tripOffer.getStatus())){
                createDeliveryRequestFromTripOffer(tripOffer.getUser(),tripOffer);
            }
            else{
                notificationService.send(AddAppNotificationDto
                        .builder()
                        .title(NotificationTitle.DELIVERY)
    ...
    public TripOfferResponse createTripOffer(CreateTripOfferRequest request, Long userId){
        SnapUser user = userService.getUserById(userId);
        PlannedTrip trip = plannedTripService.getPlannedTrip(request.getTripReference());
        ...
        Location destination = locationService.addLocation(LocationCreationDto.builder()
                ...
                .build());
        PlannedTripOffer tripOffer = tripOfferService.save(new AddTripOfferDto(
                request.getDescription(), MoneyUtilities.fromDoubleToMinor(request.getWorth()), request.getWeight(), pickup, destination,
                request.getName(), request.getPhone(), MoneyUtilities.fromDoubleToMinor(request.getOffer()), request.getNote()
        ),trip,user);
        notificationService.send(AddAppNotificationDto
                .builder()
                        .title(NotificationTitle.DELIVERY)
                        .message("There is a new delivery request for your planned trip")
                        .uid(businessService.getUserOfBusiness(trip.getBusiness()).getIdentifier())
                        .taskId(tripOffer.getTrip().getReference())
    ...
    private DeliveryRequest createDeliveryRequestFromTripOffer(SnapUser user, PlannedTripOffer offer){
        DeliveryRequest deliveryRequest = requestService.createRequest(RequestCreationDto
                .builder()
